...
const Comp = ({ slug }) => {
  ...
  const handelNextpage = () => {
    if (page >= hotalbackup.info.len - 1) { }
      ...
      routes.push(`/hotels/cityName=${cityName}&citycode=${cityCode}&checkin=${checkIn}&checkout=${checkOut}&adult=${adults}&child=${children}&roomes=${roomes}&page=${Number(page) + 1}`)
    }
  ...
  useEffect(() => {
    dispatch(getAllhotelsapi({ cityCode, checkIn, checkOut, hotelName, adults, children, page }))
  }, [])
  useEffect(() => {
    sethotalbackup(allhoteldata)
