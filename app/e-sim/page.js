...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${apilink}/matrix/plans?page=${currentPage}`);
  ...
  console.log('Render state:', { isLoading, plans, error, currentPage, lastPage });
  ...
  return (
    <>
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 my-2 lg:my-[61px]  px-[4%] lg:px-[8%] md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {Array(9)
            .fill()
            .map((_, index) => (
        ...
        <p className="mt-4">No eSIM plans available.</p>
      ) : (
        <div className="grid grid-cols-1 px-[4%] my-2 lg:my-[61px]  lg:px-[8%] md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {plans.slice(1,10)?.map((plan) =>
